...
sigma=25;
randn('seed', 2);
ima = double(imread('data/House_noisy.png'));
ima_nse = ima;% + sigma * randn(size(ima));
ima_ori = double(imread('data/House.png'));
figure('Position',[100 100  800 800]);
plotimage(ima_nse);
title(sprintf('Noisy: \n  PSNR %.2f', psnr(ima_ori, ima_nse)));
...
title(sprintf('PGPCA: \n  PSNR %.2f', psnr(ima_ori, ima_fil_PGPCA)));
sprintf('PGPCA:\n PSNR: %.2f dB \n Computing Time: %.2f s.',...
    psnr(ima_ori, ima_fil_PGPCA),global_time)
set(get(gca,'Title'),'FontSize',12);
...
title(sprintf('PLPCA: \n  PSNR %.2f', psnr(ima_ori, ima_fil_PLPCA)));
set(get(gca,'Title'),'FontSize',12);
sprintf('PLPCA:\n PSNR: %.2f dB \n Computing Time: %.2f s.', ...
    psnr(ima_ori, ima_fil_PLPCA),local_time)
...
title(sprintf('PHPCA: \n  PSNR %.2f', psnr(ima_ori, ima_fil_PHPCA)));
set(get(gca,'Title'),'FontSize',12);
sprintf('PHPCA:\n PSNR: %.2f dB \n Computing Time: %.2f s.', ...
    psnr(ima_ori, ima_fil_PHPCA),hierarchical_time)
