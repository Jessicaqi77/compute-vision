...

{
    ...
    FILE *file;
    int BytesPerPixel;
    int Sizerow=338;// change the image size
    int Sizecol=600;
    int size=Sizerow*Sizecol;
    int rep[3][4]={};
    int Size;
    int windowsize =3;//change window size
    ...
    unsigned char Imagedata1[Sizerow+position][Sizecol+position][BytesPerPixel];
...
// extension
    for (int channel=0;channel<BytesPerPixel;channel++)
    {
        ...
        for(int i=0;i<Sizerow;i++)
        {
            for (int j = 0; j < Sizecol; j++)
            {
                Imagedata1[i+position][j+position][channel] =(Imagedatanew[i][j][channel]);
                for(int m=0;m<position+1;m++){
                    Imagedata1[m][j][channel] =(Imagedatanew[m][j][channel]);
                    Imagedata1[i][m][channel] =(Imagedatanew[i][m][channel]);
                }
...
//replace the center value
    for (int i = position; i < Sizerow+position ; i++)
    {
        for (int j = position; j< Sizecol+position;j++)
        {
            int color[4][4][4]={};
