    ...
    for (int i=0;i<Size-1;i++)
    {
        if(i%2==0)
        {
            for (int j = 0; j < Size - 1; j++) {
                ...
                Imagedata[i][j + 1][0] = Imagedata[i][j + 1][0] + (7 * error) / 16;
                Imagedata[i + 1][j - 1][0] = Imagedata[i + 1][j - 1][0] + (3 * error) / 16;
                Imagedata[i + 1][j][0] = Imagedata[i + 1][j][0] + (5 * error) / 16;
                Imagedata[i + 1][j + 1][0] = Imagedata[i + 1][j + 1][0] + (1 * error) / 16;
            }
        }
        if(i%2==1)
        {
