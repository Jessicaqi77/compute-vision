...

{
    ...
    for (int channel=0; channel<3;channel++)
    {
        for(int i=1;i<Size/2;i++)
        {
            ...
            for (int p=1;p<Size/2+1;p++)
            {
                float r=(float)Size/((float)2*x);
                float small=float(p)/r;
                int a=int(small);
                float v=small-a;
                int value=(1-v)*Imagedata[i][a+Size/2-x][channel]+v*Imagedata[i][a+1+Size/2-x][channel];
                Imagedatanew[i][p][channel]=value;
            }
        }
    ...
    for (int channel=0; channel<3;channel++)
    {
        for(int i=1;i<Size/2;i++)
        {
            ...
            for (int p=0;p<Size/2;p++)
            {
                ...
                int a=int(small);
                float v=small-a;
                int value=(1-v)*Imagedata[i][a+Size/2][channel]+v*Imagedata[i][a+1+Size/2][channel];
